...
public:
    ...
    Node* solve(vector<int> &preorder,vector<int> &inorder,map<int,int> &mp,int left,int right,int &pivot)
    {
        if(left>right || pivot==preorder.size())
        {
            return NULL;
        }
        
        int ind=mp[preorder[pivot]];
        
        Node* node=new Node();
        node->data=inorder[ind];
        pivot++;
        node->left=solve(preorder,inorder,mp,left,ind-1,pivot);
        node->right=solve(preorder,inorder,mp,ind+1,right,pivot);
    ...
    Node* post_order(int pre[], int size)
    {
        ...
        int id=0;
        Node* root=solve(preorder,inorder,mp,0,size-1,id);
        return root;
    }
};
